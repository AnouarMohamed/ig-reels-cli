...
│   ├── ig\_client.py           # thin instagrapi wrapper
...
1\. \`ig\_client.py\`: wrap instagrapi. Functions: \`login(username, password)\`,

   \`load\_or\_login()\` (tries session.json first, falls back to fresh login),

   \`get\_reels\_batch(count)\` → returns list of \`{id, video\_url, caption,

   username, like\_count}\`.

2\. \`daemon.py\`: opens a UDS socket at a fixed path (e.g.

   \`/tmp/ig-reels.sock\`), listens for msgpack requests \`{"cmd": "get\_reels",
...
4\. Handle the login challenge path explicitly: if instagrapi raises a
   ...
   response (\`{"error": "challenge\_required", "detail": ...}"\`) rather than
...
2\. \`fetch.rs\`: call \`get\_reels\`, print the returned metadata as JSON/debug
